...
// Path to the Frida detection bypass script
const FRIDA_BYPASS_SCRIPT = "/data/data/com.catcher.pogoauto/files/frida-detection-bypass.js";
...
function attachToPokemonGo(pid, packageName) {
    ...
    try {
        ...
        // First load the Frida detection bypass script
        logDebug("SCRIPT", `Loading Frida detection bypass script from ${FRIDA_BYPASS_SCRIPT}`);

        try {
            // Load and execute the bypass script first
            const bypassScript = session.createScriptFromFile(FRIDA_BYPASS_SCRIPT);

            // Set up message handler for bypass script
            bypassScript.on('message', function(message) {
                if (message.type === 'send') {
                    logDebug("BYPASS_MESSAGE", `Message from bypass script: ${message.payload}`);
                } else if (message.type === 'error') {
                    logError("BYPASS_ERROR", `Error in bypass script: ${message.description}`, new Error(message.stack));
                }
            });

            // Load the bypass script
            bypassScript.load();
            logDebug("SCRIPT", "Frida detection bypass script loaded successfully");

            // Wait a moment for the bypass to take effect
            Thread.sleep(1000);

            // Now load the main Pokémon GO hook script
            logDebug("SCRIPT", `Loading main hook script from ${POKEMON_GO_HOOK_SCRIPT}`);

            try {
                const mainScript = session.createScriptFromFile(POKEMON_GO_HOOK_SCRIPT);

                // Set up message handler for main script
                mainScript.on('message', function(message) {
                    if (message.type === 'send') {
                ...
                mainScript.load();
                logDebug("SCRIPT", "Main hook script loaded successfully");

                return true;
            } catch (mainScriptError) {
                logError("SCRIPT", `Failed to load main hook script: ${mainScriptError.message}`, mainScriptError);
                return false;
            }
        } catch (bypassScriptError) {
            logError("SCRIPT", `Failed to load Frida detection bypass script: ${bypassScriptError.message}`, bypassScriptError);

            // Try to load the main script anyway
            try {
                logDebug("SCRIPT", `Bypass script failed, trying to load main hook script directly from ${POKEMON_GO_HOOK_SCRIPT}`);
                ...
                // Load the script
                script.load();
                logDebug("SCRIPT", "Hook script loaded successfully (without bypass)");
